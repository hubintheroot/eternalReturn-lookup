...
import type { Coupon, ToastData, ToastHandler, ToastStatus } from '@/shared/types';
...
export default function CouponsView(): ReactElement {
  ...
  const isUsedHandler = (usedCoupon: Coupon): void => {
    const nextData = setCouponUsed(data, usedCoupon);
    setData(nextData);
  };

  const toastHandler: ToastHandler = {
    show: (nextData: { message: string; status: ToastStatus }) => {
      setToast({ isShow: true, ...nextData });
    },
    hide: () => {
      setToast({
        isShow: false,
  ...
  function isDuplicatedCoupon(newCoupon: string): boolean {
                ...
                {data
                  ...
                  .map((coupon) => (
                    <CouponCard
                      ...
                      handler={{
                        setData: setData as Dispatch<SetStateAction<Coupon[]>>,
...
function getExpiredCoupons(data: Coupon[]): Coupon[] {
  const expired = data.filter((coupon) => !coupon.is_active);
  const sevenDaysAgo = new Date();
  sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
  const recent = expired.filter((coupon) => new Date(coupon.expires_at ?? 0) >= sevenDaysAgo);
...
function setCouponUsed(oldData: Coupon[], usedCoupon: Coupon): Coupon[] {
  const tempData = [...oldData];
  const tempCoupon = { ...usedCoupon };
  const index = tempData.indexOf(usedCoupon);
  (tempCoupon as Coupon & { is_used: boolean }).is_used = true;
