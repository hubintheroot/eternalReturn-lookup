...
`;
export const Card = styled.div<ActiveProps>`
  display: flex;
...
`;
export const Title = styled.span<ActiveProps>`
  font-size: 1.1rem;
  font-weight: 600;
  margin: 0.5rem 0;
...
`;
export const Reward = styled.p<ActiveProps>`
  font-size: 0.875rem;
  color: ${props => !props.$isActive ? '#D3DAD9' : 'rgb(107, 114, 128)'};
  margin-bottom: 1.5rem;
  line-height: 1.5;
`;
export const CodeBox = styled(InnerIcon)<ActiveProps>`
  display: flex;
  align-items: center;
...
`;
export const Code = styled.p<ActiveProps>`
  margin: 0;
  font-size: 1rem;
  flex: 1 1 0%;
...
export const CopyButton = styled(Button)`
  ...
  border: none;
`;
export const ExpiresAt = styled.div<ActiveProps>`
  font-size: 0.75rem;
  color: ${props => !props.$isActive ? 'rgb(211, 47, 47);' : 'rgb(107, 114, 128)'};
`;
