...
export default function CharacterList(): ReactElement {
  ...
  const handleSetOrd = useCallback((e: ChangeEvent<HTMLSelectElement>) => setState(e.target.value as SortState), [setState]);
  ...
  const sortData = useCallback((data: CharacterWithSkins[]): CharacterWithSkins[] => {
    if (!data) return [];
    const tempData = [...data];
    switch (sortState) {
  ...
  const filterRotation = useCallback((data: CharacterWithSkins[]): CharacterWithSkins[] => {
    if (!data) return [];
  ...
  const filterSearch = useCallback((data: CharacterWithSkins[]): CharacterWithSkins[] => {
