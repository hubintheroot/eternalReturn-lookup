...
import type { CharacterWithSkins, Skin } from '@/shared/types';
...
interface CharacterData extends CharacterWithSkins {
  loadAbleSkins: LoadableSkin[];
  textContents: TextContent[];
}

interface CharTextInfoProps {
  textContents: TextContent[];
  isLoading: boolean;
}

const CharTextInfo = memo(({ textContents, isLoading }: CharTextInfoProps): ReactElement => {
  return (
...
export default function CharacterInfo(): ReactElement | null {
  ...
  useEffect(() => {
    ...
    const loadableSkins = curCharacter.skins.filter(
      (s): s is LoadableSkin => s.full_size !== null && s.mini_size !== null,
    );
